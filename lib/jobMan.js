  ...
  function _makeWorkerDoJob(campaignId) {
    if (!_doneWorkers.length) {
      if (_currWorkerNum < _maxWorkers) {
        ...
        var workerThread = child_process.spawn('phantomjs', ['snapshot.js', campaignId]);
        workerThread.stdout.on('data', function (data) {
          console.log('stdout: ' + data);
        });
        workerThread.stderr.on('data', function (data) {
